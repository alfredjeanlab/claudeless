...
fn test_cycle_next_with_bypass() {
    ...
    assert_eq!(PermissionMode::Default.cycle_next(true), PermissionMode::AcceptEdits);
    assert_eq!(PermissionMode::AcceptEdits.cycle_next(true), PermissionMode::Plan);
...
fn test_cycle_next_from_delegate_resets_to_default() {
    ...
    assert_eq!(PermissionMode::Delegate.cycle_next(false), PermissionMode::Default);
    assert_eq!(PermissionMode::Delegate.cycle_next(true), PermissionMode::Default);
}

#[test]
fn test_cycle_next_from_dont_ask_resets_to_default() {
    // DontAsk is not in the normal cycle, goes back to Default
    assert_eq!(PermissionMode::DontAsk.cycle_next(false), PermissionMode::Default);
    assert_eq!(PermissionMode::DontAsk.cycle_next(true), PermissionMode::Default);
}
...
fn test_cycle_next_bypass_permissions_always_goes_to_default() {
    ...
    assert_eq!(PermissionMode::BypassPermissions.cycle_next(false), PermissionMode::Default);
    assert_eq!(PermissionMode::BypassPermissions.cycle_next(true), PermissionMode::Default);
}
...
fn test_value_enum_parsing() {
    ...
    assert_eq!(PermissionMode::from_str("default", true).unwrap(), PermissionMode::Default);
    assert_eq!(PermissionMode::from_str("delegate", true).unwrap(), PermissionMode::Delegate);
    assert_eq!(PermissionMode::from_str("dont-ask", true).unwrap(), PermissionMode::DontAsk);
    assert_eq!(PermissionMode::from_str("plan", true).unwrap(), PermissionMode::Plan);
}
