...
impl McpExecutor {
    ...
    fn get_or_create_client(&mut self, server_name: &str) -> Result<&mut McpClient, String> {
        // Use Entry API to avoid double lookups and unwrap
        match self.clients.entry(server_name.to_string()) {
            ...
            Entry::Vacant(e) => {
                ...
                let manager_guard = manager
                    .read()
                    .map_err(|err| format!("Failed to acquire manager lock: {}", err))?;
                ...
                let client = McpClient::spawn(&server.definition).map_err(|err| {
                    format!("Failed to spawn MCP server '{}': {}", server_name, err)
                })?;
