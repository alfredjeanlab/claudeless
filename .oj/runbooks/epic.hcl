...
#
# Examples:
#   oj run epic "Add MCP tool simulation support"
#   oj run epic "Implement streaming response playback"
#   oj run epic "Prototype new hook event format" --draft
command "epic" {
  args = "<description> [--draft]"
  run  = <<-SHELL
    if [ "${args.draft}" = "true" ]; then
      wok new epic "${args.description}" -p less -l plan:needed -l draft:needed
      oj worker start plan
      oj worker start draft
    else
      wok new epic "${args.description}" -p less -l plan:needed -l build:needed
      oj worker start plan
...
#
# Examples:
#   oj run idea "Explore better hook event simulation"
command "idea" {
  args = "<description>"
  run  = <<-SHELL
    wok new epic "${args.description}" -p less -l plan:needed
    oj worker start plan
  SHELL
}
...
#
# Examples:
#   oj run plan less-abc123
#   oj run plan less-abc123 less-def456
command "plan" {
  args = "<issues>"
  run  = <<-SHELL
...
#
# Examples:
#   oj run build less-abc123
#   oj run build less-abc123 less-def456
command "build" {
  args = "<issues>"
  run  = <<-SHELL
...
#
# Examples:
#   oj run draft less-abc123
#   oj run draft less-abc123 less-def456
command "draft" {
  args = "<issues>"
  run  = <<-SHELL
    for id in ${args.issues}; do
...
queue "plans" {
  type = "external"
  list = "wok ready -t epic,feature -l plan:needed -p less -o json"
...
queue "epics" {
  type = "external"
  list = "wok ready -t epic,feature -l build:needed -l plan:ready -p less -o json"
  take = "wok start ${item.id}"
  poll = "30s"
}
...
queue "drafts" {
  type = "external"
  list = "wok ready -t epic,feature -l draft:needed -l plan:ready -p less -o json"
  take = "wok start ${item.id}"
  poll = "30s"
}
