...
Create a test crate that simulates the `claude` CLI for integration testing. The simulator provides a controllable test double that responds to the same CLI interface as real Claude, enabling deterministic integration testing without API costs or flakiness.
