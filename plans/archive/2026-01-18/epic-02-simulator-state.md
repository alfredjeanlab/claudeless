...
Extend claudeless to simulate Claude Code's state management: the `~/.claude` directory structure, permission modes, hook protocols, and session state. This makes the simulator suitable for testing oj's integration with Claude's file-based interfaces.
